...
export default function TemplatesPage() {
  ...
  // Immediate save on blur or explicit save for updates
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="text-center space-y-4 max-w-md border border-dashed border-border p-12">
          ...
          <p className="text-muted-foreground text-sm">Select a business to manage its email templates.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden">
      ...
      <div className="h-14 border-b border-border flex items-center justify-between px-4 shrink-0 bg-card">
        <div>
          <h1 className="font-bold tracking-tight text-sm uppercase font-mono">Message Templates</h1>
        </div>
        ...
        <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
          ...
          <DialogContent className="rounded-none border-border">
            <DialogHeader>
              <DialogTitle className="font-bold">Create Template</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onCreateSubmit)} className="space-y-4 mt-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs uppercase font-mono">Template Name</FormLabel>
                      <FormControl>
                        <Input placeholder="e.g. Initial Outreach v1" className="rounded-none border-border" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs uppercase font-mono">Subject Line</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs uppercase font-mono">HTML Content</FormLabel>
                      <FormControl>
                ...
                <DialogFooter>
                  ...
                  <Button type="submit" disabled={createMutation.isPending} className="rounded-none text-xs uppercase tracking-widest">Create</Button>
                </DialogFooter>
              </form>
            </Form>
          </DialogContent>
          ...
          {selectedTemplateId && selectedTemplate ? (
            <div className="flex-1 flex flex-col p-6 gap-6 overflow-hidden">
              ...
              <div className="space-y-2 shrink-0">
                <Label className="text-xs font-mono uppercase tracking-widest text-muted-foreground">Template Name</Label>
                <Input 
              ...
              <div className="space-y-2 flex-1 flex flex-col min-h-0">
                <Label className="text-xs font-mono uppercase tracking-widest text-muted-foreground">HTML Content</Label>
                <div className="flex-1 flex border border-border">
                  ...
                  <div className="w-1/2 border-l border-border bg-card p-4 overflow-y-auto prose prose-sm dark:prose-invert max-w-none text-sm">
                    <div dangerouslySetInnerHTML={{ __html: selectedTemplate.htmlTemplate || '<div class="text-muted-foreground text-xs font-mono">PREVIEW</div>' }} />
                  </div>
                </div>
              </div>
            </div>
